...
# Directory path containing the .npz files for FUSE
dir_path = '/Users/theophile_mt/Desktop/Work/University of Zurich/CAPTAIN in Python/outputs/CAPTAIN_2_outputs/fuse_pred_20250720'
...
if results:
    ...
    # Save as RDS with "res" in the name
    budget = 0.1  # Update with your budget value
    replicates = len(results)
    rds_path = f'CAPTAIN2_FUSE_res_full_results_averaged_budget{budget}_replicates{replicates}.rds'
    ...
    with open('grid_shape_info_FUSE_res.txt', 'w') as f:
    ...
    print("Saved grid shape information to grid_shape_info_FUSE_res.txt")
    ...
    if npz_files:
        ...
        if 'protection_matrix' in data:
            ...
            plt.title('FUSE Conservation Priority Grid (First Replicate)')
            plt.xlabel('Longitude (Column Index)')
            plt.ylabel('Latitude (Row Index)')
            plt.savefig('FUSE_res_priority_grid_single_replicate.png')
            print("Saved FUSE priority grid visualization (single replicate) to FUSE_res_priority_grid_single_replicate.png")
            ...
            plt.colorbar(label='Priority Value')
            plt.title('FUSE Conservation Priority Grid (Averaged Across Replicates)')
            plt.xlabel('Longitude (Column Index)')
            plt.ylabel('Latitude (Row Index)')
            plt.savefig('FUSE_res_priority_grid_averaged.png')
            print("Saved FUSE priority grid visualization (averaged) to FUSE_res_priority_grid_averaged.png")
else:
