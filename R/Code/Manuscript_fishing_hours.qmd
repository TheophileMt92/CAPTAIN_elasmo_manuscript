...
# Load combined AIS and SAR data
load(here::here("R", "Data","combined_data_O1deg.Rdata"))

# Join overpass frequency data and compute normalised detections
combined_data_01deg <- combined_data_01deg %>%
  left_join(
    overpasses_per_cell %>% dplyr::select(lon_std, lat_std, n_overpasses, detections_per_overpass),
    by = c("lon_std", "lat_std")
  )

# Load saved models (comment out training blocks above after first run)
rf_norm_no_transform <- readRDS(here::here("R", "Data", "rf_norm_no_transform.rds"))
rf_norm_fishing_log <- readRDS(here::here("R", "Data", "rf_norm_fishing_log.rds"))
rf_norm_predictor_log <- readRDS(here::here("R", "Data", "rf_norm_predictor_log.rds"))
rf_norm_both_log <- readRDS(here::here("R", "Data", "rf_norm_both_log.rds"))
...
# Standardize to 0.1 degree grid
SAR_data_std <- aggregated_SAR_fishing %>%
  ...
  group_by(lon_std, lat_std) %>%
  summarise(
    total_presence_score = sum(total_presence_score, na.rm = TRUE),
    count = sum(count, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Join overpass frequency data
  left_join(
    overpasses_per_cell %>% dplyr::select(lon_std, lat_std, n_overpasses),
    by = c("lon_std", "lat_std")
  ) %>%
  # Compute normalised detection count
  mutate(
    detections_per_overpass = count / n_overpasses
  )
...
#  dplyr::select(
#    total_fishing_hours, 
#    detections_per_overpass,  # replaced total_presence_score
#    lon_std, lat_std, 
...
### Random forest with normalised SAR detections 

```{r}
# ---- Prepare training data with normalised SAR detections ----
# Keep the first occurrence of each coordinate pair
combined_data_01deg <- unique(combined_data_01deg, by = c("lon_std", "lat_std"))
setDT(combined_data_01deg)

# Perform the join with raster data
combined_data_with_rasters <- raster_df[combined_data_01deg,
                                        on = .(lon_std, lat_std),
                                        nomatch = 0]
...
# Prepare training data using detections_per_overpass as normalised SAR predictor
training_data <- combined_data_with_rasters %>%
  filter(has_AIS & has_SAR) %>%
  dplyr::select(
    total_fishing_hours,
    detections_per_overpass,
    lon_std, lat_std,
    dist_shore, dist_ports, bathy
  ) %>%
  na.omit() %>%
...
cat("Training data dimensions:", nrow(training_data), "rows,", ncol(training_data), "columns\n")
cat("Missing values in detections_per_overpass:", sum(is.na(training_data$detections_per_overpass)), "\n")

# ---- Train normalised random forest model ----

# No transform
#rf_norm_no_transform <- randomForest(
#  total_fishing_hours ~ detections_per_overpass + lon_std + lat_std +
#    dist_shore + dist_ports + bathy,
#  data = training_data,
#  ntree = 500,
#  importance = TRUE
#)
#saveRDS(rf_norm_no_transform, here::here("R", "Data", "rf_norm_no_transform.rds"))

# Fishing hours log only
#rf_norm_fishing_log <- randomForest(
#  log_total_fishing_hours ~ detections_per_overpass + lon_std + lat_std +
#    dist_shore + dist_ports + bathy,
#  data = training_data,
#  ntree = 500,
#  importance = TRUE
#)
#saveRDS(rf_norm_fishing_log, here::here("R", "Data", "rf_norm_fishing_log.rds"))

# Predictor log only
#rf_norm_predictor_log <- randomForest(
#  total_fishing_hours ~ log_detections_per_overpass + lon_std + lat_std +
#    dist_shore + dist_ports + bathy,
#  data = training_data,
#  ntree = 500,
#  importance = TRUE
#)
#saveRDS(rf_norm_predictor_log, here::here("R", "Data", #"rf_norm_predictor_log.rds"))

# Both log
#rf_norm_both_log <- randomForest(
#  log_total_fishing_hours ~ log_detections_per_overpass + lon_std + lat_std +
#    dist_shore + dist_ports + bathy,
#  data = training_data,
#  ntree = 500,
#  importance = TRUE
#)
#saveRDS(rf_norm_both_log, here::here("R", "Data", "rf_norm_both_log.rds"))

# ---- Evaluate normalised model ----
...
results_normalized <- evaluate_model(
  rf_norm_fishing_log, 
  validation_data, 
  log_target = TRUE
)

# ---- Generate predictions for SAR-only cells ----
prediction_data <- combined_data_with_rasters %>%
  dplyr::select(detections_per_overpass, lon_std, lat_std, dist_shore, dist_ports, bathy)

log_predictions <- predict(rf_norm_fishing_log, newdata = prediction_data)
predictions <- 10^log_predictions - 1

# Add predictions to combined_data_01deg
combined_data_01deg <- combined_data_01deg %>%
  mutate(
    predicted_fishing_hours = case_when(
      has_AIS ~ total_fishing_hours,
      has_SAR ~ predictions[match(
        paste(lon_std, lat_std),
        paste(prediction_data$lon_std, prediction_data$lat_std)
      )],
      TRUE ~ 0
    )
  )

aggregated_data_normalised <- combined_data_01deg %>%
  mutate(
    lon_05deg = floor(lon_std / 0.5) * 0.5,
    lat_05deg = floor(lat_std / 0.5) * 0.5
  ) %>%
...
save(aggregated_data_normalised,
     file = here::here("R", "Data", "Predicted_Fishing_Hours_05Deg_normalised.Rdata"))
# ---- Violin plot: observed vs predicted ----
...
violin_plot <- ggplot(plot_data, aes(x = type, y = hours, fill = type)) +
  ...
  scale_x_discrete(labels = c("ais_fishing_hours" = "AIS Data",
                               "sar_predicted_hours" = "SAR Predictions (normalised)")) +
  labs(title = "Comparison of AIS fishing hours and normalised SAR predicted fishing hours",
       subtitle = "AIS data for AIS-covered areas, normalised predictions for SAR-only areas",
       x = "", y = "Fishing hours (log scale)", fill = "Type") +
  theme_minimal() +
...
predicted_plot <- ggplot() +
  ...
  labs(title = "Global fishing hours (0.1 degree resolution)",
       subtitle = "AIS data and normalised random forest predictions from SAR data",
       x = "Longitude", y = "Latitude") +
  theme(
    legend.position = "bottom",
    legend.direction = "horizontal",
...
ggsave(here::here("R", "Outputs", "predicted_plot_SAR_normalized.png"),
       plot = predicted_plot,
       width = 12, height = 8, dpi = 300)
```

### Comparison of transformations - normalised models
```{r}
validation_data_norm <- combined_data_with_rasters %>%
  mutate(
    data_category = case_when(
      has_AIS & has_SAR ~ "Both AIS and SAR",
      has_AIS & !has_SAR ~ "Only AIS",
...
results_norm_no_transform <- evaluate_model(rf_norm_no_transform, validation_data_norm)
results_norm_fishing_log <- evaluate_model(rf_norm_fishing_log, validation_data_norm, log_target = TRUE)
results_norm_predictor_log <- evaluate_model(rf_norm_predictor_log, validation_data_norm)
results_norm_both_log <- evaluate_model(rf_norm_both_log, validation_data_norm, log_target = TRUE)
...
results_comparison <- data.frame(
  ...
  No_Transform = round(c(
    results_norm_no_transform$`Mean absolute error`,
    results_norm_no_transform$`Root mean squared error`,
    results_norm_no_transform$`Mean absolute percentage error`,
    results_norm_no_transform$`Median absolute error`,
    results_norm_no_transform$`R-squared`,
    results_norm_no_transform$`Adjusted R-squared`,
    results_norm_no_transform$`Mean of residuals`,
    results_norm_no_transform$`Standard deviation of residuals`
  ), 2),
  Fishing_Log = round(c(
    results_norm_fishing_log$`Mean absolute error`,
    results_norm_fishing_log$`Root mean squared error`,
    results_norm_fishing_log$`Mean absolute percentage error`,
    results_norm_fishing_log$`Median absolute error`,
    results_norm_fishing_log$`R-squared`,
    results_norm_fishing_log$`Adjusted R-squared`,
    results_norm_fishing_log$`Mean of residuals`,
    results_norm_fishing_log$`Standard deviation of residuals`
  ), 2),
  Predictor_Log = round(c(
    results_norm_predictor_log$`Mean absolute error`,
    results_norm_predictor_log$`Root mean squared error`,
    results_norm_predictor_log$`Mean absolute percentage error`,
    results_norm_predictor_log$`Median absolute error`,
    results_norm_predictor_log$`R-squared`,
    results_norm_predictor_log$`Adjusted R-squared`,
    results_norm_predictor_log$`Mean of residuals`,
    results_norm_predictor_log$`Standard deviation of residuals`
  ), 2),
  Both_Log = round(c(
    results_norm_both_log$`Mean absolute error`,
    results_norm_both_log$`Root mean squared error`,
    results_norm_both_log$`Mean absolute percentage error`,
    results_norm_both_log$`Median absolute error`,
    results_norm_both_log$`R-squared`,
    results_norm_both_log$`Adjusted R-squared`,
    results_norm_both_log$`Mean of residuals`,
    results_norm_both_log$`Standard deviation of residuals`
  ), 2)
)

kable(results_comparison, format = "html", digits = 2,
      col.names = c("Metric", "No transform", "Fishing hours log",
                    "Predictor log", "Both log"),
      caption = "Model performance comparison - normalised SAR detections per overpass") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                full_width = FALSE) %>%
...
Four model variants were compared to identify the optimal transformation strategy 
for normalised SAR detections per overpass. The results clearly favour 
log-transforming the response variable (fishing hours).

The fishing hours log and both-log models achieve the highest R-squared (0.77) 
and the lowest MAPE (85.64% and 86.25% respectively), compared to over 1,780% 
for models without response transformation. The median absolute error is similarly 
improved, dropping from ~33 hours to ~12 hours with log transformation of the 
response. Log-transforming the predictor (detections per overpass) alone provides 
no meaningful improvement over the no-transform version across any metric.

The fishing hours log model (log_total_fishing_hours ~ detections_per_overpass) 
is selected as the final model. It performs marginally better than the both-log 
variant on MAPE (85.64% vs 86.25%) and RMSE (1413 vs 1415), and is simpler to 
interpret since the predictor remains on its original scale.

Compared to the previous non-normalised model (R-squared = 0.82, MAPE = 69.71%), 
performance is slightly reduced. This is expected: normalisation by overpass 
frequency removes a spurious signal that the original model was exploiting, namely 
systematic variation in detection counts driven by uneven satellite coverage rather than true differences in fishing intensity. The normalised model is therefore 
preferred on methodological grounds despite the modest reduction in predictive 
accuracy.
...
results_rf_fishing_log <- evaluate_model(rf_norm_fishing_log, validation_data, log_target = TRUE)
...
# Create the table
table_output <- kable(feature_importance, format = "html", digits = 2, 
