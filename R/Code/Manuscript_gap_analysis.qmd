...
# Save results to CSV
write.csv(results, here::here("R","Data","species_mpa_coverage_NT.csv"), row.names = FALSE)
```
...
# Create base theme with larger text
...
# Save ecoregion map
ggsave(here("R","Outputs", "ecoregion_NT_coverage_map.png"), eco_map, width = 12, height = 8, dpi = 300)
...
# Save EEZ map
ggsave(here("R","Outputs", "eez_NT_coverage_map.png"), eez_map, width = 12, height = 8, dpi = 300)
...
ggsave(here("R","Outputs", "top_ecoregions_NT_coverage.png"), top_eco_plot, width = 10, height = 8, dpi = 300)
...
ggsave(here("R","Outputs", "bottom_ecoregions_NT_coverage.png"), bottom_eco_plot, width = 10, height = 8, dpi = 300)
...
# Save top EEZs plot
ggsave(here("R","Outputs", "top_eezs_NT_coverage.png"), top_eez_plot, width = 10, height = 8, dpi = 300)
```
...
print(ses_eco_map)
ggsave(here("R","Outputs", "ecoregion_SES_map_NT_clean.png"), 
...
print(top_ses_plot)
ggsave(here("R","Outputs", "top_ecoregions_SES_NT_clean.png"), 
       top_ses_plot, width = 10, height = 8, dpi = 300)

cat("Bottom 20 Ecoregions by Mean SES:\n")
print(bottom_ses_plot)
ggsave(here("R","Outputs", "bottom_ecoregions_SES_NT_clean.png"), 
...
## Relate the percentage of species range overlapped by MPAs to the IUCN Red List status
...
summary_stats_nt <- results_IUCN_filtered %>%
  ...
  summarise(
    ...
    median = median(NT, na.rm = TRUE),
    sd = sd(NT, na.rm = TRUE),
    min = min(NT, na.rm = TRUE),
    max = max(NT, na.rm = TRUE),
    Q1 = quantile(NT, 0.25, na.rm = TRUE),
    Q3 = quantile(NT, 0.75, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(factor(IUCN_status, levels = iucn_order))
...
# Check for zeros
print("\n2. SPECIES WITH ZERO COVERAGE:")
zero_coverage <- results_IUCN_filtered %>%
  group_by(IUCN_status) %>%
  summarise(
    n_zeros = sum(NT == 0, na.rm = TRUE),
    pct_zeros = round(100 * sum(NT == 0, na.rm = TRUE) / n(), 2),
    .groups = "drop"
  ) %>%
  arrange(factor(IUCN_status, levels = iucn_order))
print(zero_coverage)
...
overall_skewness <- skewness(results_IUCN_filtered$NT, na.rm = TRUE)

# Calculate skewness by category
skewness_stats <- results_IUCN_filtered %>%
  group_by(IUCN_status) %>%
  summarise(
    skewness = skewness(NT, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(factor(IUCN_status, levels = iucn_order))
...
skewness_stats <- rbind(overall_row, skewness_stats)
...
cat("NON-PARAMETRIC STATISTICAL TESTS\n")

# 1. Jonckheere-Terpstra test
library(DescTools)
cat("\n1. JONCKHEERE-TERPSTRA TEST (for ordered alternatives):\n")
jt_result <- JonckheereTerpstraTest(results_IUCN_filtered$NT, 
                                     results_IUCN_filtered$iucn_GE,
                                     alternative = "decreasing")
print(jt_result)

# 2. Spearman correlation
cat("\n2. SPEARMAN RANK CORRELATION:\n")
spearman_result <- cor.test(results_IUCN_filtered$iucn_GE, 
                             results_IUCN_filtered$NT, 
                             method = "spearman")
print(spearman_result)

# 3. Kolmogorov-Smirnov test (LC vs CR)
cat("\n3. KOLMOGOROV-SMIRNOV TEST (LC vs CR):\n")
ks_result <- ks.test(
  results_IUCN_filtered$NT[results_IUCN_filtered$IUCN_status == "LC"],
  results_IUCN_filtered$NT[results_IUCN_filtered$IUCN_status == "CR"]
)
print(ks_result)
...
for (tau in quantiles) {
  cat(paste("Fitting quantile regression for tau =", tau, "...\n"))
  
  tryCatch({
    qr_model <- rq(NT ~ iucn_GE, tau = tau, data = results_IUCN_filtered)
    ...
    temp_result <- data.frame(
      ...
      Intercept_SE = coef_table[1, 2],
      Intercept_t = coef_table[1, 3],
      Intercept_p = coef_table[1, 4],
      Slope = coef_table[2, 1],
      Slope_SE = coef_table[2, 2],
      Slope_t = coef_table[2, 3],
      Slope_p = coef_table[2, 4]
    )
    
    qr_results <- rbind(qr_results, temp_result)
    cat("  Success!\n")
    
  }, error = function(e) {
    cat(paste("  Error at tau =", tau, ":", e$message, "\n"))
    cat("  Skipping this quantile.\n")
  })
}

# Create cleaned version (exclude NA p-values)
qr_clean <- qr_results[!is.na(qr_results$Slope_p), ]

if (nrow(qr_clean) > 0) {
  
  # Add significance indicator
  qr_clean$Significant <- ifelse(qr_clean$Slope_p < 0.05, "Yes*", "No")
  
  # Round for display
  qr_summary_clean <- qr_clean
  qr_summary_clean$Slope <- round(qr_summary_clean$Slope, 3)
  qr_summary_clean$Slope_SE <- round(qr_summary_clean$Slope_SE, 3)
  qr_summary_clean$Slope_t <- round(qr_summary_clean$Slope_t, 3)
  qr_summary_clean$Slope_p <- round(qr_summary_clean$Slope_p, 4)
  
  # Create table for display
  simple_table <- qr_summary_clean[, c("Quantile", "Tau", "Slope", "Slope_SE", 
                                        "Slope_t", "Slope_p", "Significant")]
  
  # Rename columns for better display
  colnames(simple_table) <- c("Quantile", "τ", "Slope", "SE", "t", "p-value", "Significant")
  
  simple_kable = kable(simple_table, 
        format = "html",
        col.names = c("Quantile", "τ", "Slope", "SE", "t", "p-value", "Significant"),
        caption = "Quantile regression results for current no-take MPA coverage by IUCN threat status", 
        digits = 2,
        row.names = FALSE,
    ...
    column_spec(6, width = "90px") %>%
    column_spec(7, width = "100px", 
                color = ifelse(simple_table$Significant == "Yes*", "#D81E05", "gray50")) %>%
    row_spec(0, bold = TRUE, background = "#f0f0f0") %>%
    row_spec(which(simple_table$Significant == "Yes*"), 
             background = "#fff5f5") %>%
    footnote(general = "Bootstrap standard errors with 500 replications. Significant quantiles (p < 0.05) are highlighted.",
             general_title = "Note:")
  
  # Save as PNG
  save_kable(simple_kable,
             file = here::here("R","Outputs", "simple_kable_quantile results.png"),
             zoom = 2)
  #======================
  # FIGURE 1: Distribution plot + Quantile slopes
  #====================
  
  # Panel A: Distribution plot (matching your original)
  notake_mpa_iucn <- results_IUCN_filtered %>%
    dplyr::select(species, IUCN_status, NT) %>%
    rename(protection = NT)
  
  # Prepare data for quantile regression lines on categorical x-axis
  # We need to convert IUCN_status factor levels to numeric positions for plotting
  qr_lines_categorical <- data.frame()
  
  for (tau in quantiles) {
    qr_model <- rq(NT ~ iucn_GE, tau = tau, data = results_IUCN_filtered)
    
    # Predict for each IUCN category (0-4)
    pred_data <- data.frame(iucn_GE = 0:4)
    pred_data$predicted <- predict(qr_model, newdata = pred_data)
    pred_data$IUCN_status <- c("LC", "NT", "VU", "EN", "CR")
    pred_data$quantile <- tau
    pred_data$quantile_label <- paste0("Q", tau * 100)
    
    qr_lines_categorical <- rbind(qr_lines_categorical, pred_data)
  }
  
  # Add significance status
  qr_lines_categorical <- qr_lines_categorical %>%
    left_join(qr_summary_clean[, c("Tau", "Significant")], 
              by = c("quantile" = "Tau"))
  
  # Panel A: Distribution with quantile regression lines
  panel_a <- ggplot() +
    # Raw data points
    geom_jitter(data = notake_mpa_iucn,
    ...
    geom_line(data = qr_lines_categorical,
              aes(x = factor(IUCN_status, levels = iucn_order),
                  y = predicted,
                  group = quantile_label,
                  linetype = Significant),
              color = "black",
              linewidth = 0.8) +
    # Add quantile labels on the LEFT (LC side)
    geom_text(data = qr_lines_categorical %>% 
                filter(IUCN_status == "LC"),
              aes(x = 0.8, y = predicted, 
  ...
  # Panel B: Quantile slopes
  panel_b <- ggplot(qr_summary_clean, aes(x = Tau, y = Slope)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray50", linewidth = 1) +
    geom_line(linewidth = 1.2, color = "black") +
    geom_point(aes(color = Significant), size = 4) +
    geom_ribbon(aes(ymin = Slope - 1.96 * Slope_SE, 
                    ymax = Slope + 1.96 * Slope_SE),
                alpha = 0.3, fill = "steelblue") +
    scale_color_manual(values = c("No" = "gray60", "Yes*" = "#D81E05"),
                       name = "Significant at α=0.05") +
    labs(
      x = "Quantile (τ)",
      y = "Slope coefficient\n(% change per IUCN category)"
    ) +
    theme_minimal() +
    theme(
    ...
    scale_x_continuous(breaks = qr_summary_clean$Tau, 
                       labels = qr_summary_clean$Quantile)
  
  # Combine panels
  fig1_combined <- ggpubr::ggarrange(
    panel_a,
    panel_b,
    ncol = 2,
    nrow = 1,
  ...
  ggsave(
    filename = here::here("R","Outputs", "FigS1_quantile_regression.png"),
    plot = fig1_combined,
    width = 14,
    height = 6,
    dpi = 300,
...
} # Close the if (nrow(qr_clean) > 0) statement
...
rdplot_points <- ggplot(notake_mpa_iucn, 
  ...
  stat_summary(fun.data = function(x) {
    ...
    se_val <- sd(x, na.rm = TRUE) / sqrt(sum(!is.na(x)))  # SE = SD / sqrt(n)
...
output_path <- here::here("R","Outputs", "Fig_1_combined_points_4.png")
...
knitr::include_graphics(here::here("R","Outputs", "Fig_1_combined_points_4.png"))
```
