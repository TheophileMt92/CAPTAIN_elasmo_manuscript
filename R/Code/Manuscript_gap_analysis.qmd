...
if (nrow(qr_clean) > 0) {
  ...
  ggsave(
    filename = here::here("R","Outputs", "FigS1_distribution_and_slopes.png"),
    plot = fig1_combined,
    width = 14,
    height = 6,
    dpi = 300,
  ...
  #======================
  # FIGURE 2: Scatterplot with fitted quantile regression lines
  #======================
  
  cat("\n")
  cat(rep("=", 80), "\n", sep="")
  cat("CREATING FIGURE 2: SCATTERPLOT WITH QUANTILE REGRESSION LINES\n")
  cat(rep("=", 80), "\n", sep="")
  
  # Get fitted quantiles
  fitted_quantiles <- qr_summary_clean$Tau
  
  # Create data frame for fitted lines
  fitted_lines <- data.frame()
  
  for (tau in fitted_quantiles) {
    qr_model <- rq(NT ~ iucn_GE, tau = tau, data = results_IUCN_filtered)
    pred_data <- data.frame(iucn_GE = seq(0, 4, by = 0.1))
    pred_data$predicted <- predict(qr_model, newdata = pred_data)
    pred_data$quantile <- tau
    pred_data$quantile_label <- paste0("Q", tau * 100)
    
    fitted_lines <- rbind(fitted_lines, pred_data)
  }
  ...
  qr_scatter <- ggplot() +
    # Raw data points
    geom_jitter(data = results_IUCN_filtered,
                aes(x = iucn_GE, y = NT),
                width = 0.15, height = 0,
                alpha = 0.2, size = 1.5, color = "gray60") +
    # Quantile regression lines
    geom_line(data = fitted_lines,
              aes(x = iucn_GE, y = predicted, 
                  group = quantile_label,
                  color = quantile,
                  linetype = Significant),
              linewidth = 1.2) +
    # Quantile labels
    geom_text(data = fitted_lines %>% 
                filter(iucn_GE == max(iucn_GE)),
              aes(x = iucn_GE + 0.15, y = predicted, 
                  label = quantile_label,
                  color = quantile),
    ...
    scale_linetype_manual(values = c("No" = "dashed", "Yes*" = "solid"),
                          name = NULL,
                          labels = c("No" = "Not significant (p ≥ 0.05)", 
    ...
    coord_cartesian(xlim = c(-0.2, 4.8), clip = "off") +
  ...
  ggsave(
    filename = here::here("R","Outputs", "Fig2_quantile_regression_scatterplot.png"),
    plot = qr_scatter,
    width = 10,
    height = 7,
    dpi = 300,
  ...
  cat("Figure 2 saved to: Fig2_quantile_regression_scatterplot.png\n")
  
} else {
  cat("\nERROR: No valid quantile regression results obtained.\n")
}
